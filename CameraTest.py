...
modelSaveSrc = 'D:/aiData'

morphemesDict = saveCSV.csvLoad(os.path.join(modelSaveSrc,'morphemesSentenceDict.csv'))
morphemesList = list(morphemesDict.keys())

model_key_path = os.path.join(modelSaveSrc,'KeyDetectmodel.pt')
model_keyLstm_path = os.path.join(modelSaveSrc,'KeyDetectLSTMmodel.pt')

model_key = torch.load(model_key_path)
model_key.eval()
model_lstm = torch.load(model_keyLstm_path)
model_lstm.eval()
...
failFlag = 0
cap = cv2.VideoCapture('NIA_SL_SEN1950_REAL04_R.mp4')
    ...
    while cap.isOpened():
        ...
        if (len(myHands) > 2):
            ...
            for h in myHands[-1]:
                if failFlag == 0:
                    # print(h)
                    cv2.circle(image, (h[0], h[1]), 5, (0, 0, 255), -1, cv2.LINE_AA)
        ...
        for i in range(int(len(myHands) / 2)):
            ...
            idx += 1
        if(len(handkeypoints)>30):
            predic_result = predictProcess(handkeypoints)
            print("result:", morphemesList[predic_result.exp().argmax().item()],"possible:",round(predic_result.exp().max().item()*100,2),'%')
            resultList.append(morphemesList[predic_result.exp().argmax().item()])
            handkeypoints = dict()
            handsType = []
            myHands = []
            errorFrame = []
            myHand = []
            cap.set(cv2.CAP_PROP_POS_FRAMES, 8*len(resultList))
            continue
